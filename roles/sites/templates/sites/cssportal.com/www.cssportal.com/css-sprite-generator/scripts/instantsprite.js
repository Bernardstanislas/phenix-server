...
sprite.fileReaderOpts = {
	...
	on: {
		...
		beforestart: function(file) {
			...
			var cantLoad = file.type == 'image/tiff';
			if (cantLoad) {
...
sprite.setrules = function() {
	...
	elements.exportHtml.text(outputHtml);
	elements.exportCss.text(outputCss);
...
sprite.init = function() {
	...
	elements.exportImageNewWindow.click(function() {
		if (!$(this).hasClass("disabled")) {
			...
			var imageType = elements.optionExport.filter(":checked").val();
			var str = resultBase64;
			var res = str.replace("image/png", "image/" + imageType);
			download(res, "sprite." + imageType, "image/" + imageType);
		}
		return false;
	});
